...
## 闭合算法,取值问题
## 递归向下降解获取分块的问题
## 向下降解的程度, 25%,30%,45%,分3个区间.
## 需要新的rawscan方法
## 找出连续的像素行
## 有这么几种情况,<10% 且不能分行的作为同一行合并处理,标记为W
## <30%的认为是 sidebar 作为纯粹图像处理,标记为S
## >30%且可分多行的作为column处理,因此不限定于多少column,标记为C
## 先检查sidebar,把非两边的sidebar都转换成W